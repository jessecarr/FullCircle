...
export default function TimesheetPage() {
  ...
  // Handle time edit (admin only)
  const handleStartTimeEdit = (date: string, field: 'time_in' | 'time_out', currentValue: string | null) => {
    if (!isAdmin) return
    setEditingCell({ date, field })
    // Convert ISO time to HH:MM format for input
    if (currentValue) {
      const time = new Date(currentValue)
                        ...
                        <tbody>
                          {week.days.map((day, dayIndex) => (
                            <tr key={dayIndex} className={`${dayIndex % 2 === 0 ? 'bg-background' : 'bg-muted/30'} hover:bg-muted/50 transition-colors`}>
                              ...
                              <td className="px-3 py-2 text-center border border-border relative">
                                {editingCell?.date === day.date && editingCell?.field === 'time_in' ? (
                                  <div className="relative inline-block">
                                    <span className="text-primary font-medium">{editValue ? formatTimeFromHHMM(editValue) : 'Select time'}</span>
                                    <TimePicker
                                      value={editValue}
                                      onChange={setEditValue}
                                      onSave={() => handleSaveEdit(day.date, 'time_in')}
                                  ...
                                  <span 
                                    ...
                                    onClick={() => handleStartTimeEdit(day.date, 'time_in', day.timesheet?.time_in || null)}
                                  >
                              ...
                              <td className="px-3 py-2 text-center border border-border relative">
                                {editingCell?.date === day.date && editingCell?.field === 'time_out' ? (
                                  <div className="relative inline-block">
                                    <span className="text-primary font-medium">{editValue ? formatTimeFromHHMM(editValue) : 'Select time'}</span>
                                    <TimePicker
                                      value={editValue}
                                      onChange={setEditValue}
                                      onSave={() => handleSaveEdit(day.date, 'time_out')}
                                      onCancel={handleCancelEdit}
                                    />
                                  </div>
                                ) : (
                                  <span 
                                    className={isAdmin ? 'cursor-pointer hover:bg-muted px-2 py-1 rounded' : ''}
                                    onClick={() => handleStartTimeEdit(day.date, 'time_out', day.timesheet?.time_out || null)}
                                  >
