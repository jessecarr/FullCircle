...
export default function OrderingPage() {
  ...
  const handleSearchChange = (value: string) => {
    ...
    if (scanMode) return // In scan mode, don't search — wait for Enter
  ...
  const handleSearchKeyDown = (e: React.KeyboardEvent) => {
    // Scan mode: Enter adds barcode to scanned list
    if (scanMode && e.key === 'Enter') {
      e.preventDefault()
      const value = searchQuery.trim()
      if (value.length > 0) {
        setScannedItems(prev => {
          if (prev.includes(value)) return prev // skip duplicates
          return [...prev, value]
        })
        setSearchQuery('')
      }
  ...
  const handleAnalyzeScanned = async () => {
    ...
    try {
      ...
      // Merge with existing results or create new
      setResult(prev => {
        if (!prev) return data
